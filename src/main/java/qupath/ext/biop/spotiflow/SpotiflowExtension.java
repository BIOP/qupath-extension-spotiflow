...
public class SpotiflowExtension implements QuPathExtension, GitHubProject {
    ...
    protected static String getExtensionVersion(){
        ...
        try {
            List<URL> manifestList = Collections.list(SpotiflowExtension.class.getClassLoader().getResources("META-INF/MANIFEST.MF"))
                    .parallelStream()
                    .filter(e -> e.toString().contains("qupath-extension-spotiflow") && !e.toString().contains("javadoc"))
